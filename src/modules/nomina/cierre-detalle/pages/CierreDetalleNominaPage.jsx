// src/modules/nomina/cierre-detalle/pages/CierreDetalleNominaPage.jsx
import { useState, useEffect, useCallback } from "react";
import { useParams } from "react-router-dom";
import { obtenerCierreNominaPorId, obtenerCliente } from "../api/cierreDetalle.api";
import { validarAccesoNomina } from "../utils/cierreDetalleHelpers";
import CierreInfoBar from "../components/CierreInfoBar";
import CierreProgresoNomina from "../components/CierreProgresoNomina";

/**
 * Página de detalle de cierre de nómina
 * Muestra toda la información y progreso del cierre
 */
const CierreDetalleNominaPage = () => {
  ...
  useEffect(() => {
    ...
    if (!validarAccesoNomina(usuario)) {
  ...
  const refrescarCierre = useCallback(async () => {
    if (cierreId) {
      try {
        ...
        console.log('🔄 [CierreDetalleNominaPage] Cierre actualizado:', cierreActualizado.estado);
      } catch (error) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        console.log('✅ [CierreDetalleNominaPage] Datos iniciales cargados', { 
          cierre: cierreObj, 
          cliente: clienteObj 
        });
      } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center">
        <div className="bg-red-900/20 border border-red-700 rounded-lg p-8 max-w-md text-center">
          ...
          <p className="text-gray-300">No tienes permisos para acceder al área de Nómina.</p>
        </div>
      </div>
    );
  }
...
export default CierreDetalleNominaPage;
