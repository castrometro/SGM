// src/pages/ClienteDetalleContabilidadModuleDemo.jsx
import { useState } from 'react';
import { Link, Routes, Route, useNavigate, useParams } from 'react-router-dom';
import { ClienteDetalleContabilidadPage } from '../modules/contabilidad/cliente-detalle';
...
const DemoLayout = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (inputValue.trim()) {
      navigate(`/dev/modules/cliente-detalle-contabilidad/demo/${inputValue.trim()}`);
    }
  };
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-900 text-gray-100">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="text-sm font-semibold text-gray-900 block">
                  DEMO: Módulo Detalle de Cliente de Contabilidad
                </span>
                <span className="text-xs text-gray-700">
                  Vista completa con KPIs contables y acciones rápidas
                </span>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="flex items-center gap-2">
              ...
              <input
                ...
                className="w-24 px-3 py-1.5 bg-white border border-gray-300 rounded-md text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-purple-600"
                min="1"
              />
              <button
                type="submit"
                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-purple-600 text-white rounded-md text-sm font-medium hover:bg-purple-700 transition-colors"
              >
                <FiSearch size={14} />
                Cargar
              </button>
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        ...
        {/* Contenido - ClienteDetalleContabilidadPage */}
        <main className="flex-grow container mx-auto p-6 pt-32">
          <ClienteDetalleContabilidadPage />
        </main>
...
/**
 * Demo del módulo Detalle de Cliente de Contabilidad refactorizado
 * Muestra la página de detalle con Layout completo
 * Usa el usuario real del localStorage
 */
const ClienteDetalleContabilidadModuleDemo = () => {
  return (
    <Routes>
...
export default ClienteDetalleContabilidadModuleDemo;
