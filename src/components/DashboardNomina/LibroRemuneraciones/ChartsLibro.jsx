...
const ChartsLibro = ({
  ...
  const safeCategoriasComparacion = Array.isArray(categoriasComparacion) ? categoriasComparacion : [];
  ...
  // Fallbacks seguros para evitar errores de .map sobre undefined
  const safeActiveConceptosBarData = Array.isArray(activeConceptosBarData) ? activeConceptosBarData : [];
  ...
  const safeActiveDonutData = Array.isArray(activeDonutData) ? activeDonutData : [];
      ...
      <div className={`bg-gray-900/60 rounded-xl p-4 border border-gray-800 flex flex-col transform transition-all duration-700 ease-out ${chartsEntered ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-[0.97] translate-y-2'}`}>
        ...
        <div className="flex-1 h-80">
          <ResponsiveContainer width="100%" height="100%">
            {(!selectedCat && safeCategoriasComparacion.length) ? (
              <BarChart data={safeCategoriasComparacion} layout="vertical" margin={{ top:8,right:16,left:0,bottom:8 }}>
              ...
              <BarChart data={safeActiveConceptosBarData} layout="vertical" margin={{ top:8,right:16,left:0,bottom:8 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#1f2937" />
                <XAxis type="number" stroke="#9ca3af" tickFormatter={formatearNumero} tick={{ fontSize: 11 }} />
                ...
                <Bar dataKey="value" radius={[0,6,6,0]} isAnimationActive animationDuration={600} animationEasing="ease-out">
                  {safeActiveConceptosBarData.map((entry,i) => {
                    const palette = ['#0d9488','#14b8a6','#2dd4bf','#0ea5e9','#6366f1','#8b5cf6','#d946ef','#f59e0b','#f97316','#f43f5e'];
  ...
  {!selectedCat && safeCategoriasComparacion.length===0 && <div className="text-center text-xs text-gray-500 py-4">Sin datos</div>}
  {selectedCat && safeActiveConceptosBarData.length === 0 && <div className="text-center text-xs text-gray-500 py-4">No hay elementos visibles.</div>}
      </div>
      {/* DONUT */}
      <div className={`bg-gray-900/60 rounded-xl p-4 border border-gray-800 flex flex-col transform transition-all duration-700 ease-out delay-75 ${chartsEntered ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-[0.97] translate-y-2'}`}>
        ...
        <div className="flex-1 flex items-center justify-center">
          <ResponsiveContainer width="100%" height={selectedCat ? 320 : 300}>
            <PieChart>
              <Pie data={safeActiveDonutData} dataKey="value" nameKey="name" innerRadius={70} outerRadius={110} paddingAngle={2} labelLine={false} label={false} isAnimationActive animationDuration={600} animationEasing="ease-out">
                {safeActiveDonutData.map((entry,i) => (
