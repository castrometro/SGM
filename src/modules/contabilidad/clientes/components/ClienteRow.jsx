// src/modules/contabilidad/clientes/components/ClienteRow.jsx
import { useEffect, useState } from "react";
import { useNavigate } from "react-router-dom";
import { motion } from "framer-motion";
import { obtenerResumenContabilidad } from "../api/clientes.api";
import { ANIMATION_CONFIG, URLS } from "../constants/clientes.constants";
import EstadoBadge from "./EstadoBadge";
import ClienteActions from "./ClienteActions";

/**
 * 📋 Componente ClienteRow - Fila/Card de cliente para Contabilidad
...
const ClienteRow = ({ cliente, areaActiva, index = 0 }) => {
  ...
  useEffect(() => {
    const fetchResumen = async () => {
      setCargando(true);
      try {
        const data = await obtenerResumenContabilidad(cliente.id);
        setResumen(data);
      } catch (error) {
        setResumen({ ultimo_cierre: null, estado_cierre_actual: null, usuario_cierre: null });
        console.error("Error al cargar resumen de contabilidad:", error);
      } finally {
        setCargando(false);
      }
    };
  ...
  const handleVerCierres = () => {
    navigate(URLS.CIERRES_CLIENTE(cliente.id));
  };
