...
const FiniquitosCard = ({
  ...
  useEffect(() => {
    if (cierreId && !activityLogger.current) {
      activityLogger.current = createActivityLogger(cierreId, 'finiquitos');
  ...
  useEffect(() => {
    console.log('🎯 Finiquitos useEffect polling - estado actual:', estado, 'deberiaDetener:', deberiaDetenerPolling);
    
    // Detener polling si se debe detener globalmente
    if (deberiaDetenerPolling && pollingRef.current) {
      console.log('🛑 [FiniquitosCard] Deteniendo polling - señal global de parada');
    ...
    if (estado === "en_proceso" && !pollingRef.current && onActualizarEstado && !deberiaDetenerPolling) {
      console.log('🔄 Iniciando polling para monitorear procesamiento de finiquitos...');
      ...
      pollingRef.current = setInterval(async () => {
        contadorPolling++;
        try {
          console.log(`📡 Finiquitos Polling #${contadorPolling} - Verificando estado...`);
          await onActualizarEstado();
          contadorErrores = 0;
        } catch (pollError) {
          contadorErrores++;
          console.error(`❌ Error en polling Finiquitos #${contadorPolling} (${contadorErrores}/3):`, pollError);
      ...
      console.log(`✅ Finiquitos Estado cambió a "${estado}" o detención solicitada - deteniendo polling`);
  ...
  const handleSeleccionArchivo = async (e) => {
    ...
    setError("");
    try {
      console.log('📁 Iniciando subida de archivo Finiquitos:', archivo.name);
      await onSubirArchivo(archivo);
      console.log('✅ Archivo Finiquitos subido exitosamente');
      
      if (onActualizarEstado) {
        console.log('🔄 Forzando actualización de estado post-subida Finiquitos...');
        setTimeout(() => {
          onActualizarEstado();
        }, 500);
      }
      ...
      console.error('Error subiendo archivo Finiquitos:', err);
    }
  };
  ...
  return (
    <div className={`bg-gray-800 p-4 rounded-xl shadow-lg flex flex-col gap-3 ${isDisabled ? "opacity-60 pointer-events-none" : ""}`}>
      ...
      <a
        href={descargarPlantillaFiniquitos()}
        download
        ...
        onClick={async () => {
          if (activityLogger.current) {
            await activityLogger.current.logDownloadTemplate('finiquitos');
          }
        }}
      >
        <Download size={16} />
      ...
      {(estado === "no_subido" || estado === "pendiente") && (
        <div className="text-xs text-blue-400 mt-1 bg-blue-900/20 p-2 rounded">
          📋 <strong>Formato requerido:</strong> AAAAAMM_finiquitos_RUT.xlsx
          <br />
          <span className="text-blue-300">Ejemplo: 202508_finiquitos_12345678.xlsx</span>
        </div>
      )}
