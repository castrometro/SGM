// src/modules/nomina/cliente-detalle/pages/ClienteDetalleNominaPage.jsx
import { useEffect, useState } from "react";
import { useParams, useNavigate } from "react-router-dom";
import { 
  obtenerCliente, 
  obtenerKpisNominaCliente, 
...
import KpiResumenNomina from "../components/KpiResumenNomina";
import ClienteActionButtons from "../components/ClienteActionButtons";

/**
 * 📄 Página de Detalle de Cliente de Nómina
 * Muestra información completa del cliente y sus KPIs
 * Valida acceso al área de Nómina antes de cargar datos
 */
const ClienteDetalleNominaPage = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      ...
      try {
        ...
        // 2. Cargar cliente
        console.log('🔍 ClienteDetalleNomina - Cargando cliente:', id);
        const clienteData = await obtenerCliente(id);
        setCliente(clienteData);

        // 3. Cargar KPIs de nómina
        let resumenData = null;
        try {
          console.log('🔍 ClienteDetalleNomina - Obteniendo KPIs...');
          const kpisData = await obtenerKpisNominaCliente(id);
          console.log('🔍 ClienteDetalleNomina - KPIs obtenidos:', kpisData);
          
          if (kpisData?.tieneCierre) {
            resumenData = procesarDatosResumen(kpisData);
            console.log('🔍 ClienteDetalleNomina - Datos procesados:', resumenData);
          }
        } catch (e) {
          console.warn('⚠️ ClienteDetalleNomina - Fallo KPIs, usando fallback:', e);
        }

        // Fallback a resumen básico si no hay KPIs
        if (!resumenData) {
          console.log('🔍 ClienteDetalleNomina - Usando resumen básico...');
          resumenData = await obtenerResumenNomina(id);
          console.log('🔍 ClienteDetalleNomina - Resumen básico:', resumenData);
        }

        setResumen(resumenData);
      } catch (err) {
        console.error("❌ ClienteDetalleNomina - Error:", err);
      ...
      <div className="flex items-center justify-center h-64">
        <div className="max-w-md mx-auto text-center p-8">
          ...
          <p className="text-gray-400 mb-6">
            No tienes permisos para acceder a detalles de clientes de Nómina.
          </p>
          <button
  ...
  return (
    <div className="text-white space-y-6">
      ...
      {/* KPIs del cierre */}
      {resumen && <KpiResumenNomina resumen={resumen} />}
...
export default ClienteDetalleNominaPage;
