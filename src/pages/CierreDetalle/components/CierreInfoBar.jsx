...
const CierreInfoBar = ({ cierre, cliente, onCierreActualizado, tipoModulo = "contabilidad" }) => {
  ...
  const manejarActualizarEstado = async () => {
    ...
    try {
      ...
      // Manejar diferentes APIs según el tipo de módulo
      if (tipoModulo === "contabilidad") {
        const { actualizarEstadoCierre } = await import('../../../api/contabilidad');
      ...
      } else if (tipoModulo === "nomina") {
        // TODO: Implementar API de nómina
        console.log('API de nómina aún no implementada');
        alert('La funcionalidad de nómina está en desarrollo');
      } else if (tipoModulo === "rrhh") {
        // TODO: Implementar API de RRHH
        console.log('API de RRHH aún no implementada');
        alert('La funcionalidad de RRHH está en desarrollo');
      }
    } catch (error) {
  ...
  const consultarProgreso = async (taskId) => {
    try {
      const { obtenerProgresoTarea } = await import('../../../api/contabilidad');
  ...
  const iniciarFinalizacion = async () => {
    ...
    try {
      const { finalizarCierre } = await import('../../../api/contabilidad');
  ...
  return (
    <div className="bg-gray-800 px-8 py-6 rounded-xl shadow flex flex-wrap items-center gap-6 mb-10 w-full">
      ...
      <div className="flex items-center gap-3">
        ...
        {/* Botones para visualización de datos - Para nómina */}
